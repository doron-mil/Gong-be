...
function persistStaticData(aNewCoursesTemplateJson = null, aNewGong = null, aUpdatedPermissions = null) {
  ...
  if (aUpdatedPermissions) {
    _.forEach(currentStaticData.permissions, permission => {
      ...
      if (foundPermission) {
        ...
        throw new Error(`PersistManager=>persistStaticData failed to find Permissions object : ${permission.action}`);
      }
    });
  }
...
function validatePermissions(aPermissionsArrayJson) {
  ...
  _.forEach(aPermissionsArrayJson, (permission) => {
    ...
    if (permission.roles.some(role => !['dw', 'teacher', 'admin', 'dev'].includes(role))) {
      throw new Error(`Roles must be one of ['dw', 'teacher', 'admin', 'dev'] for permission : ${permission.action}`);
    }
  });
}
