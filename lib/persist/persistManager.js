...
function persistStaticData(aNewCoursesTemplateJson = null) {
  ...
  return persistDataObject('staticData', currentStaticData).then(() => utilsManager.updateCourses());
}
    ...
    fs.readdir(langsBundlesJsonFilesPath, async (error, files) => {
      ...
      try {
        ...
        files.forEach((file) => {
          ...
          coursesNamesArray.forEach((newCourseName) =>
            _.set(translation, ['general', 'typesValues', 'courses', newCourseName], ''));
