...
const getErrorLog = (aError) => `Error : ${aError.message}. Error output:\n ${aError.stack}`;

const reformatManualGongJobsForLog = (aJob) => {
  const formattedTime = moment(aJob.time).format('YY/MM/DD.HH:mm');
  const isManualStr = aJob.isManual ? 'Manual' : ' Automatic';
  const dataStrigified = JSON.stringify(aJob.data);
  const aJobStatus = aJob.status.description;
  const retFormattedGong = `\n\t\tJob time is ${formattedTime}. Is ${isManualStr}.`
    + ` Status is ${aJobStatus}.\n\t\t\t Data : ${dataStrigified} `;
...
logger.gongsManager = createLogger({
  ...
  transports: [
    new transports.File({ filename: 'logs/gongManager.log', label: 'gongManager' }),
    new (transports.Console)({
      timestamp: tsFormat,
      colorize: true,
...
logger.scheduleManager = createLogger({
  ...
  transports: [
    new transports.File({ filename: 'logs/scheduleManager.log', label: 'scheduleManager' }),
    new (transports.Console)({
      timestamp: tsFormat,
      colorize: true,
      format: gongConsoleFormat(),
